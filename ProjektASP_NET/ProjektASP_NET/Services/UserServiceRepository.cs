...
namespace ProjektASP_NET.Services
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<IEnumerable<User>> GetAllAsync(int offset, int fetch)
        {
            ...
            return _collection.Find(filter).Skip(offset).Limit(fetch).ToList();
        }
        ...
        public async Task UpdateAsync(string id, User model)
        {
            await _collection.FindOneAndUpdateAsync(
                ...
                Builders<User>.Update
                    .Set(c => c.FirstName, model.FirstName)
                    .Set(c => c.LastName, model.LastName)
                    .Set(c => c.Username, model.Username)
                    .Set(c => c.Email, model.Email)
                    .Set(c => c.Password, model.Password),
                new FindOneAndUpdateOptions<User> { ReturnDocument = ReturnDocument.After });
        }

        public async Task<User> GetByEmailAsync(string email)
        {
            return await _collection.Find(model => model.Email == email).FirstOrDefaultAsync();
        }
    }
}
